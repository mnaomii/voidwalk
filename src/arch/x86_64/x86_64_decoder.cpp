...
// Decodes one x86/x86-64 instruction: prefixes, REX, opcode (including the 0F and
// 0F 38 / 0F 3A escapes), ModRM, SIB, displacement and immediates, then appends the
// rendered Instruction to `decodedInstructions`.
//
// Returns the offset just past the instruction, or `address` unchanged if the
// instruction ran past end-of-file. Never reads more than 15 bytes.
