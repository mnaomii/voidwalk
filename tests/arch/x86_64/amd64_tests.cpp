...
//
#include "tests/arch/x86_64/x86_64_base.hpp"
...
class AMD64_Tests : public Tests {
    ...
    void testGroup3ImmediateWidth() {
        ...
        eq({0x48, 0xf7, 0xc7, 0x3f, 0x00, 0x00, 0x00}, "TEST RDI, 0x3f", 7, "48 f7 c7 (libc test rdi,0x3f)");
        eq({0xf7, 0xc0, 0x01, 0x00, 0x00, 0x00},       "TEST EAX, 0x1",  6, "f7 /0 no REX takes imm32");
        eq({0x66, 0xf7, 0xc0, 0x01, 0x00},             "TEST AX, 0x1",   5, "66 f7 /0 opsize takes imm16");
        eq({0xf6, 0xc0, 0x01},                         "TEST AL, 0x1",   3, "f6 /0 takes imm8");
    }
    ...
    void runAll() {
        ...
        running("testGroup3ImmediateWidth");testGroup3ImmediateWidth();
    }
...
// AMD64_Tests's constructor, and the shared Tests counters aggregate the results.
void run_amd64_tests() { AMD64_Tests t; }
